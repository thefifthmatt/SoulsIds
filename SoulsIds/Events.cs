...
namespace SoulsIds
{
    public class Events
    {
        ...
        // Parse a command so it can be added. Does not support parameters or initializations.
        ...
        public (EMEVD.Instruction, List<EMEVD.Parameter>) ParseAddArg(string add, int addIndex)
        {
            ...
            for (int i = 0; i < addArgs.Count; i++)
            {
                ...
                if (arg.StartsWith("X"))
                {
                    ...
                    ps.Add(p);
                    args.Add(ParseArg("0", argTypes[i]));
                }
                else
                {
                    args.Add(ParseArg(arg, argTypes[i]));
                }
            }
